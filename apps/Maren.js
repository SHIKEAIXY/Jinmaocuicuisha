    ...
    constructor() {
		super({
			...
			rule: [
                ...
                {
                    /** 命令正则匹配（欢迎投稿） */
                    reg: '^#?(.*)闭嘴(.*)|(.*)滚(.*)|(.*)傻逼(.*)|(.*)傻(.*)|(.*)日你(.*)|(.*)草(.*)|(.*)操(.*)|(.*)艹(.*)|(.*)bz(.*)|(.*)sb(.*)|(.*)gun(.*)|(.*)cnm(.*)|(.*)草泥马(.*)|(.*)操你妈(.*)|(.*)草你妈(.*)|(.*)你麻痹(.*)|(.*)你妈逼(.*)|(.*)日你妈(.*)|(.*)日尼玛(.*)|(.*)nm(.*)|(.*)rnm(.*)|(.*)nmb(.*)|(.*)NMB(.*)|(.*)RNM(.*)|(.*)NM(.*)|(.*)SB(.*)|(.*)GUN(.*)|(.*)CNM(.*)|(.*)尼玛(.*)$|(.*)你妈(.*)|(.*)你妹(.*)|(.*)尼妈(.*)|(.*)你玛(.*)|(.*)尼妹(.*)|(.*)傻子(.*)|(.*)智障(.*)|(.*)ZZ(.*)|(.*)zz(.*)|(.*)垃圾(.*)|(.*)lj(.*)|(.*)呆逼(.*)|(.*)二臂(.*)|(.*)死逼(.*)|(.*)穷逼(.*)|(.*)笨蛋(.*)|(.*)白痴(.*)|(.*)王八(.*)|(.*)三八(.*)|(.*)贱人(.*)|(.*)逼样(.*)|(.*)DBF(.*)|(.*)小人(.*)|(.*)坏蛋(.*)|(.*)王八蛋(.*)|(.*)狼心狗肺(.*)|(.*)狗仗人势(.*)|(.*)禽兽不如(.*)|(.*)狼狈为奸(.*)|(.*)衣冠禽兽(.*)|(.*)猪狗不如(.*)|(.*)忘恩负义(.*)|(.*)为虎作伥(.*)|(.*)恩将仇报(.*)|(.*)指鹿为马(.*)|(.*)贼眉鼠眼(.*)|(.*)粉墨登场(.*)|(.*)鼠目寸光(.*)|(.*)祸国殃民(.*)|(.*)卑鄙无耻(.*)|(.*)丧心病狂(.*)|(.*)丧尽天良(.*)|(.*)狼心狗肺(.*)|(.*)吃里扒外(.*)|(.*)心怀叵测(.*)|(.*)井底之蛙(.*)|(.*)厚颜无耻(.*)|(.*)小肚鸡肠(.*)|(.*)dullness(.*)|(.*)Two-arm(.*)|(.*)Death(.*)|(.*)Poor(.*)|(.*)stupid(.*)|(.*)idiot(.*)|(.*)Wang Ba(.*)|(.*)38(.*)|(.*)Slut(.*)|(.*)Forced sampledullness(.*)|(.*)Two-arm(.*)|(.*)Poor(.*)|(.*)stupid(.*)|(.*)idiot(.*)|(.*)Wang Ba(.*)|(.*)Slut(.*)|(.*)Forced sample(.*)|(.*)DBF(.*)|(.*)villain(.*)|(.*)Bad guys(.*)|(.*)Wang Baegg(.*)|(.*)Wolf Heart Dog Lung(.*)|(.*)Dog Fighting Momentum(.*)|(.*)Birds and beasts are inferior to(.*)|(.*)Gangsterism(.*)|(.*)Clothed beasts(.*)|(.*)Pigs and dogs are inferior to(.*)|(.*)ungratefulness(.*)|(.*)for the tiger(.*)|(.*)Eun will take revenge(.*)|(.*)refers to a deer as a horse(.*)|(.*)くそ(.*)|(.*)ばか(.*)|(.*)ブス(.*)|(.*)ちくしょう (.*)|(.*)アホ (.*)|(.*)愚か者(.*)|(.*)おろかもの(.*)|(.*)はくち(.*)|(.*)はくち(.*)|(.*)ちくしょう(.*)|(.*)ぼけ(.*)|(.*)きちがい(.*)|(.*)げひん(.*)|(.*)いやらしい(.*)|(.*)できそこない(.*)|(.*)でぶでぶ、ぶよぶよ(.*)|(.*)见にくい颜(.*)|(.*)へんたい(.*)|(.*)くず(.*)|(.*)LJ(.*)$',
                    /** 执行方法 */
                    fnc: 'Maren'
                }
            ]
