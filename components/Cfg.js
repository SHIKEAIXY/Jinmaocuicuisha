...
initCfg() {
    let path = `${Plugin_Path}/Cfg/`
    let pathDef = `${Plugin_Path}/Cfg/default/`
    const files = fs.readdirSync(pathDef).filter(file => file.endsWith('Sys.yaml'))
  ...
  /** 用户配置 */
  getConfig (app, name) {
	return this.getYaml(app, name, 'Cfg')
  }

  saveConfig (app, name, data) {
    return this.save(app, name, 'Cfg',data)
  }
  ...
  modify(name, key, value) {
    let path = `${Plugin_Path}/Cfg/${name}.yaml`
